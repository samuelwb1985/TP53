...
outFile = sys.argv[3] # matrix of mutations with mutation context added as last column
outSummaryFile = sys.argv[4] # summary of mutation contexts for each change pattern in inFile
...
header = ["Sample", "Chrom", "Pos", "Ref", "Alt", "Source"] #inputFile.readline().rstrip("\n").split("\t")
...
for rowIndex in range(1, len(inMatrix)):
	chrom = inMatrix[rowIndex][1].replace("chr", "")
	site  = inMatrix[rowIndex][2]
	ref   = inMatrix[rowIndex][3]
	alt   = inMatrix[rowIndex][4] #.split(".") # get mutation chromosome and site
	...
	if "N" in mutContext:
		inMatrix[rowIndex].append("N-found-not-determinable")
		continue
...
for rowIndex in range(1, len(inMatrix)):
	...
	if mutationContext != "NotApp-Indel" and mutationContext != "N-found-not-determinable": # if this is a valid mutation context (i.e. not a "C>CA" type indel or other unusable case)
		...
		if mutationContext in possibleContexts:
			...
			mutationContextMatrix[rowIndexInMutationContextMatrix][colIndexInMutationContextMatrix] += 1 # increment by 1
                        ...
                        print("WARNING: The following row didn't have a valid mutation context:")
                        print inMatrix[rowIndex]

                        invalidContexts += 1

print("Number of mutations with invalid contexts was " + str(invalidContexts))
